...
class  GOMDataset(VisionDataset):
    ...
    def __getitem__(self, index: int):
        ...
        if os.path.exists(f"{dataset_cache}/{cache_file}"):
            ...
            raise Exeption(f"{cache_file} not found!")
        ...
        if self.transforms is not None:
            ...
            head_bbox = target['boxes'][torch.nonzero(target['objects'] == 0).item()]
            cx, cy, w, h = head_bbox
...
class  HGOMDataset(VisionDataset):
    ...
    def __init__(
        ...
        self._walker = pd.DataFrame(rows)
        self._heads = pd.read_csv(heads_csv, header=None)
        self._heads.columns = ["video_file", "frame_number", "cx", "cy", "w", "h"]
...
def build_dataset(
    ...
    transforms = make_gom_transforms("test", test_scale=-1)

    if config["directories"].get("head_output", None):
        print("Using HGOMDataset")
        return HGOMDataset(
