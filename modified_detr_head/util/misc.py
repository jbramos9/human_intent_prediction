...
def collate_fn_with_head(batch):
    batch = list(zip(*batch))
    batch[0] = nested_tensor_from_tensor_list(batch[0])
...
class NestedTensor(object):
    ...
    # def cat_with(self, other: 'NestedTensor') -> 'NestedTensor':
    #     merged = torch.cat()
    #     return NestedTensor()
...
def get_gazed_objects(outputs, targets, device):
    ...
    gazes = [
        torch.masked_select(target["objects"], target["isgazed"] > 0).to(device)
        for target in targets
    ]
    # Add default gazed class ID 16 to account for null class

    gazes = [
        gaze.to(device) if gaze.nelement() else 16 * torch.ones(1).to(device)
        for gaze in gazes
    ]

    targets = torch.stack(gazes).squeeze(1)

    return gazed_object, targets

def get_head_box(outputs):
    objects = outputs["pred_object_logits"].argmax(-1)
    idx = objects == 0
    bbox = outputs["pred_boxes"][idx]
    return bbox
